...
const path = path$6;
const { app, ipcMain, ipcRenderer, shell } = require$$3$1;
...
const _Donwload = class _Donwload {
  constructor(winId, option) {
    __publicField(this, "savePath");
    __publicField(this, "options");
    __publicField(this, "win");
    __publicField(this, "progress", null);
    __publicField(this, "cacheItem", {});
    __publicField(this, "tmpPath", "");
    this.savePath = path$6.join(
      this.dirIsExists(path$6.join("C:/Users/33597/Music/test/video", option.itag)),
      option.fileName + option.format
    );
    this.tmpPath = this.savePath + ".tmp";
    this.options = option;
    this.win = require$$3$1.BrowserWindow.fromId(winId);
    if (this.win === null) {
  ...
  dirIsExists(path2) {
    if (!fs$3.existsSync(path2)) {
      fs$3.mkdirSync(path2);
    }
    return path2;
  }
  initEvent() {
    require$$3$1.app.once("before-quit", () => {
      fs$3.copyFileSync(this.savePath, this.tmpPath);
      this.reloadCache();
    });
  ...
  saveDownloadItem(item) {
    ...
    this.cacheItem.path = item.getSavePath();
    this.cacheItem.eTag = item.getETag();
    this.cacheItem.urlChain = item.getURLChain();
    this.cacheItem.length = item.getTotalBytes();
    this.cacheItem.lastModified = item.getLastModifiedTime();
    ...
    return (_, state) => {
      ...
      if (state === "progressing") {
        if (currentItem.isPaused()) {
          ...
          this.cacheItem.speedBytes = offset - lastBytes;
          this.cacheItem.offset = offset;
          lastBytes = offset;
          const progress = currentItem.getReceivedBytes() / currentItem.getTotalBytes() * 100;
          (_a = this.progress) == null ? void 0 : _a.call(this, progress, this.cacheItem.speedBytes);
        }
      }
    };
  }
  ...
  start() {
    var _a;
    const cacheItem = (_a = _Donwload.downloadMrg[this.options.thid]) == null ? void 0 : _a[this.options.itag];
    const isTmp = fs$3.existsSync(this.tmpPath);
    if (cacheItem) {
      if (fs$3.existsSync(cacheItem.path) || isTmp) {
        if (cacheItem.offset === cacheItem.length) {
          console.log("文件已存在");
          return;
        }
        if (isTmp) {
          fs$3.renameSync(this.tmpPath, this.savePath);
        }
        this.win.webContents.session.createInterruptedDownload({
          path: cacheItem.path,
          urlChain: cacheItem.urlChain,
          offset: cacheItem.offset,
          // 下载断点开始位置
...
__publicField(_Donwload, "downloadMrg", store.get("downloadMrg"));
let Donwload = _Donwload;
main.initialize();
const createWindow = () => {
  ...
  require$$3$1.ipcMain.on("start_download", (_, data) => {
    console.log("start_download", data);
    const downlaodController = new Donwload(win.id, data);
    downlaodController.setProgressCallback((progress, speedBytes) => {
      console.log({ progress, speedBytes });
    });
    downlaodController.start();
  });
  require$$3$1.ipcMain.handle("getAppPath", () => {
    return require$$3$1.app.isPackaged ? path$6.dirname(require$$3$1.app.getPath("exe")) : require$$3$1.app.getAppPath();
  });
  win.webContents.on("did-attach-webview", (event, webview) => {
    console.log(path$6.join(__dirname, "dist-electron/index.js"));
    webview.webpreferences = {
      preload: path$6.join(__dirname, "dist-electron/index.js")
    };
  });
  global.shareObject = {
    webviewPreload: path$6.join(__dirname, "preload.js")
  };
  if (process.env.NODE_ENV !== "development") {
    win.loadFile(path$6.join(__dirname, "./index.html"));
